    ...
    return (
      <div className="flex min-h-[80vh] items-center justify-center px-4">
        <div className="flex flex-col items-center gap-4 text-center max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">Vérifiez votre boîte email pour confirmer votre adresse. Redirection dans 3 secondes...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={e => { void handleRegister(e); }} className="space-y-4">
          <div>
            ...
            <Input required value={fields.fullName} onChange={e => setFields(f => ({...f, fullName: e.target.value}))} placeholder="Jean Mukendi" />
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-medium">{t('auth.email')} *</label>
            <Input type="email" required autoComplete="email" value={fields.email}
              onChange={e => setFields(f => ({...f, email: e.target.value}))} placeholder="jean@exemple.com" />
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-medium">Téléphone (optionnel)</label>
            <Input type="tel" value={fields.phone} onChange={e => setFields(f => ({...f, phone: e.target.value}))} placeholder="+243 81 XXX XXXX" />
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-medium">{t('auth.password')} *</label>
            <Input type="password" required autoComplete="new-password" value={fields.password}
              onChange={e => setFields(f => ({...f, password: e.target.value}))} placeholder="8 caractères minimum" />
          </div>
        ...
        <p className="mt-6 text-center text-sm text-muted-foreground">
          {t('auth.haveAccount')}{' '}
          <Link to={'/login' as never} className="font-semibold text-rdc-blue hover:underline">{t('auth.login')}</Link>
        </p>
      </div>
    </div>
  );
