...
export const Route = createFileRoute('/vols/arrivees')({
  ...
  head: () => ({
    meta: [
      ...
      { name: 'description', content: 'Tableau des arrivées en temps réel à l\'Aéroport International de N\'djili (FIH). Vols, compagnies et statuts.' },
    ],
  }),
});
      ...
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-rdc-green/10">
              <PlaneLanding size={18} className="text-rdc-green" />
            </div>
            <h1 className="font-display text-2xl font-bold text-rdc-anthracite md:text-3xl">
              {t('flights.arrivals')}
            </h1>
          </div>
          <p className="mt-1 text-sm text-muted-foreground">{t('vols.arrivees.subtitle')}</p>
        </div>
