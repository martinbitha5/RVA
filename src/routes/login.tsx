    ...
    <div className="flex min-h-[80vh] items-center justify-center py-16 px-4">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={e => { void handleLogin(e); }} className="space-y-4">
          <div>
            ...
            <Input type="email" required autoComplete="email" value={email}
              onChange={e => setEmail(e.target.value)} placeholder="jean@exemple.com" />
          </div>
          <div>
        ...
        <p className="mt-6 text-center text-sm text-muted-foreground">
          {t('auth.noAccount')}{' '}
          <Link to={'/inscription' as never} className="font-semibold text-rdc-blue hover:underline">{t('auth.register')}</Link>
        </p>
      </div>
    </div>
  );
