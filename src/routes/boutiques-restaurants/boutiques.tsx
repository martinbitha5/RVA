...
export const Route = createFileRoute('/boutiques-restaurants/boutiques')({
  ...
  head: () => ({
    meta: [
      ...
      { name: 'description', content: "Artisanat congolais, duty free, presse et souvenirs à l'Aéroport International de N'djili." },
    ],
  }),
});

const PAGE_DATA = CONCESSIONS.filter(
  (c) => c.category === 'boutique' || c.category === 'duty_free'
);

const AVAILABLE_ZONES = [
  ...new Set(PAGE_DATA.map((c) => c.zoneKey)),
] as ZoneKey[];

/* ─── Card (répertoire style) ────────────────────────────────────────── */
function BoutiqueCard({ c }: { c: Concession }) {
  ...
  return (
    ...
    >
      <div className="relative h-44 overflow-hidden flex-shrink-0">
        ...
        {c.category === 'duty_free' && (
          <span className="absolute left-3 top-3 bg-rdc-yellow px-2 py-0.5 text-[10px] font-bold text-rdc-anthracite">
            HORS TAXES
          </span>
        )}
      </div>
...
function BoutiquesPage() {
  ...
  return (
    <main id="main-content">
      ...
      {/* ── Hero ────────────────────────────────────────────────── */}
      <div className="relative overflow-hidden bg-[#1A2640]">
        <img
        ...
        />
        <div
          className="absolute inset-0 bg-[#1A2640]"
        ...
        <div className="container relative z-10 py-14 md:py-20">
          <nav className="mb-4 flex items-center gap-1.5 text-[11px] font-medium text-white/40">
            ...
            <ChevronRight size={10} />
            <span className="text-white/70">{t('shops.shops')}</span>
          </nav>
          ...
          <h1 className="font-display text-5xl font-bold text-white md:text-6xl">Boutiques</h1>
          <p className="mt-3 max-w-sm text-white/60">
            Artisanat congolais authentique, hors-taxes, presse et cadeaux avant votre départ.
          </p>
          <p className="mt-4 text-sm font-medium text-rdc-yellow">
      ...
      <div className="container py-8 md:py-10">
        ...
        >
          {filtered.map((c) => <BoutiqueCard key={c.id} c={c} />)}
        </div>
      </div>
    </main>
  );
